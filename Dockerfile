...
RUN if [ "${RUNNER}" != "github" ]; then \
    ...
    && cd /tmp && cat LibreOffice_24.8.7_Linux_x86-64_deb.tar.gz* | tar zxf - \
    && cd /tmp/LibreOffice_24.8.7.2_Linux_x86-64_deb/DEBS \
    && dpkg -i *.deb \
    && cd /usr/share/fonts/chinese && mkfontscale && mkfontdir && fc-cache -fv \
    && ln -sf /opt/libreoffice24.8 /opt/libreoffice \
